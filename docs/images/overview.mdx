...
For registries using self-signed or internal CA certificates, point `ca_certs` to a PEM file:
...
Auth, TLS, and insecure settings live together in each registry entry:
...
These settings can also be set per-sandbox via the SDK:
...
SDK settings are merged with the global config — both sources are additive.
